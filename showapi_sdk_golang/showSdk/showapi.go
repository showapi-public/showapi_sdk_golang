...
func (sa *ShowApi) Get(url string) (map[string]interface{}, error) {
	sa.sum()
	req := httplib.Get(url + "?" + sa.Encode())
	var saRet map[string]interface{}
	req.ToJson(&saRet)
	sa.Del("showapi_sign")
	return sa.unmarshal(saRet)
}

//以post方式发送一个调用请求,返回接口body,上传文件需用此方式
func (sa *ShowApi) Post(url string) (map[string]interface{}, error) {
	sa.sum()
...
func (sa *ShowApi) sum() {
	...
	t := time.Now().In(prc)
	//sa.Set("showapi_timestamp", fmt.Sprintf("%d%d%d%d%d%d", t.Year(), t.Month(), t.Day(), t.Hour(), t.Minute(), t.Second()))
